...
#Not sure why this chunk is preventing me from rendering
```
...
sunapee$ice_off_doy <- as.numeric(as.character(sunapee$ice_off_doy))

sunapee_plot <- sunapee |>
  ggplot(aes(x = year, y = ice_off_doy))+
  geom_line()+
  geom_point()+
  labs(title = "Lake Sunapee Ice Off", x = "Year", y = "Ice Off")
sunapee_plot
```

# How does ice cover change over time at Lake Sunapee?
Ice cover has decreased over time since 1869 at Lake Sunapee
...
**Answer 2:**
I expect it to be slightly negative because of the overall decline.
...
```{r}
#sunapee <- sunapee$ice_off_doy <- as.numeric(as.character(sunapee$ice_off_doy))
...
```{r}
fit$coefficients[318.94856]
fit$coefficients[-0.10602]
```
...
**Answer 3:**
The slope is negative which matches by prediction from question 2.
...
**Answer 4:**
The R2 is 0.2055. Since this value is low, it is apparent that year and ice_off_doy are not strongly related.
...
**Answer 6:**
The residuals are slightly normally distributed. Although the data does look a little skewed left, I believe you could make the argument that it looks normally distributed.
...
**Answer 7:**
According to the summary the p-value of the lm model is 2.504e-09. Therefore it suggests that there is a statistically significant relationship.
...
**Answer 9:**
I am very comfortable, in Bio Stat last semester one of the main focuses was linear models.
**Question 10:**
...
sunapee_plot <- sunapee |>
  ...
  geom_line()+
  geom_point()+
  geom_abline(intercept = 318.94856, slope = -0.10602)+
  labs(title = "Lake Sunapee Ice Off", x = "Year", y = "Ice Off")
```
...
**Answer 11:**
In 2040, the predicted ice-off day will be 102.6622

The 5% interval is -19.42732 and the 95% is 14.15137

The width of the confidence interval is 33.6 days

```{r}
predict_2040 <- predict(fit, newdata = data.frame(year = 2040))
upper_res <- quantile(sunapee$residual, probs = c(0.95), na.rm = TRUE)
lower_res <- quantile(sunapee$residual, probs = c(0.05), na.rm = TRUE)

width <- (predict_2040 + upper_res) - (predict_2040 + lower_res)
...
**Answer 12:**
The data shows us that ice-off day of the year is NOT changing more quickly in recent years. Although the y-intercept of data from after 1970 is much higher, the slope of both data sets is only separated by about 0.2. The two plots I created show very similar trends therefore indicating the data does not show ice-off day is changing faster in recent years.
```{r}
sunapee_old <- sunapee |>
  filter(year < 1970)
sunapee_new <- sunapee|>
...
fit_old <- lm(ice_off_doy ~ year, data = sunapee_old)
fit_new <- lm(ice_off_doy ~ year, data = sunapee_new)
#fit_old
#fit_new
sunapee_plot_old <- sunapee_old |>
  ggplot(aes(x = year, y = ice_off_doy))+
  geom_line()+
  geom_point()+
  geom_abline(intercept = 325.5421, slope = -0.1097)+
  labs(title = "Lake Sunapee Ice Off", x = "Year", y = "Ice Off")
sunapee_plot_new <- sunapee_new |>
  ggplot(aes(x = year, y = ice_off_doy))+
  geom_line()+
  geom_point()+
  geom_abline(intercept = 711.9216, slope = -0.3024)+
  labs(title = "Lake Sunapee Ice Off", x = "Year", y = "Ice Off")
sunapee_plot_new
sunapee_plot_old
#summary(fit_old)
